...
export default function AbsenGuruPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/presensi/absenguru?${params.toString()}`);
      setRows(res?.data || []);
  ...
  useEffect(() => {
    apiFetch("/data/guru")
      .then((res) => setGuruOptions(res?.data || []))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.id_user) {
      Swal.fire("Lengkapi data", "Pilih guru terlebih dahulu.", "warning");
      return;
    }
    setSaving(true);
    try {
      ...
      Swal.fire({
        title: "Berhasil",
        text: editing ? "Absensi guru diperbarui." : "Absensi guru ditambahkan.",
        icon: "success",
        timer: 1500,
  ...
  const handleDelete = async (row: AbsenGuru) => {
    const nama = row.users?.DataUser?.nama_lengkap || "guru ini";
  ...
  return (
    <AppShell title="Absen Guru">
      ...
      <Modal
        title={editing ? "Edit Absensi Guru" : "Tambah Absensi Guru"}
        open={modalOpen}
        onClose={() => setModalOpen(false)}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Guru</label>
            <select
              required
              disabled={editing}
              value={form.id_user}
