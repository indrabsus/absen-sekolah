...
export default function RekapGuruPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/presensi/rekap-bulanan-guru?bulan=${bulan}&tahun=${tahun}`);
      setHari(res?.hari || []);
      setRows(res?.data || []);
    } catch (err) {
  ...
  return (
    <AppShell title="Rekap Bulanan Guru">
      <div className="hidden print:mb-2 print:block">
        ...
        <p className="text-center text-xs">
          Rekap Kehadiran Guru — {BULAN_LIST[bulan - 1]} {tahun}
        </p>
      </div>
